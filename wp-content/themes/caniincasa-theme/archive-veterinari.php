<?php
/**
 * Template for Veterinari Archive
 *
 * @package Caniincasa
 */
...
<main id="main-content" class="site-main archive-strutture archive-veterinari">

    <!-- Archive Header -->
    <div class="archive-header">
        <div class="container">
            <h1 class="archive-title">Veterinari</h1>
            <p class="archive-description">Trova strutture veterinarie in tutta Italia</p>
        </div>
    </div>
            ...
            <aside class="strutture-filters-sidebar">
                ...
                <div class="filters-box">
                    <div class="filters-header">
                        <h3>🔍 Filtra Veterinari</h3>
                        <button class="filters-reset" id="reset-filters">
                            Resetta
                        </button>
                    </div>

                    <form id="veterinari-filters-form" class="filters-form">
                        ...
                        <input type="hidden" name="action" value="filter_veterinari" />
                        <input type="hidden" name="nonce" value="<?php echo wp_create_nonce( 'filter_veterinari_nonce' ); ?>" />
            ...
            <div class="strutture-main-content">
                ...
                <div class="results-header">
                    <span class="results-count">
                        <span id="veterinari-count"><?php echo $wp_query->found_posts; ?></span> veterinari trovati
                    </span>
                ...
                <!-- Loading Overlay -->
                <div id="veterinari-loading" class="loading-overlay" style="display: none;">
                    <div class="loader"></div>
                    <p>Caricamento veterinari...</p>
                </div>

                <!-- Veterinari Grid -->
                <div id="veterinari-grid" class="strutture-grid view-grid">
                    <?php
                    if ( have_posts() ) :
                        while ( have_posts() ) :
                            the_post();
                ...
                <div id="veterinari-pagination-container">
                    <?php
...
jQuery(document).ready(function($) {
    ...
    const $form = $('#veterinari-filters-form');
    const $grid = $('#veterinari-grid');
    const $loading = $('#veterinari-loading');
    const $count = $('#veterinari-count');
    const $paginationContainer = $('#veterinari-pagination-container');
    ...
    // Load veterinari AJAX
    function loadVeterinari(page = 1) {
        $loading.show();
    ...
    $(document).on('click', '.strutture-pagination a.pagination-link', function(e) {
        ...
        $('html, body').animate({
            scrollTop: $('.archive-veterinari').offset().top - 100
        }, 500);
    });
});
</script>
