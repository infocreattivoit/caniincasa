...
jQuery(document).ready(function($) {
	$('#dog-profile-form').on('submit', function(e) {
		...
		$submit.prop('disabled', true).text('<?php esc_js_e( 'Salvataggio...', 'caniincasa-my-dog' ); ?>');
		...
		$.ajax({
			...
			success: function(response) {
				if (response.success) {
					...
					setTimeout(function() {
						window.location.href = '<?php echo esc_url( home_url( '/i-miei-cani/' ) ); ?>';
					}, 1500);
				} else {
					$message.addClass('error').text(response.data.message).fadeIn();
					$submit.prop('disabled', false).text('<?php echo $is_edit ? esc_js__( 'Aggiorna Profilo', 'caniincasa-my-dog' ) : esc_js__( 'Crea Profilo', 'caniincasa-my-dog' ); ?>');
				}
			},
			error: function() {
				$message.addClass('error').text('<?php esc_js_e( 'Errore di connessione. Riprova.', 'caniincasa-my-dog' ); ?>').fadeIn();
				$submit.prop('disabled', false).text('<?php echo $is_edit ? esc_js__( 'Aggiorna Profilo', 'caniincasa-my-dog' ) : esc_js__( 'Crea Profilo', 'caniincasa-my-dog' ); ?>');
			}
		});
	});
